...
- Bot seating retries use native `Seat:Sit` without repositioning attached rigs. `WorldBindings` checks actual SeatWeld connections in addition to humanoid seating state; ambient movement stops while attached and exits detach the bot before moving it clear of the chair. Authored seat transforms and normal SeatWeld offsets remain unchanged.
...
- Eliminated human spectators can leave a continuing match via Jump or an authored Exit/Leave control. The server validates the current MatchId and zero hearts, ejects them, releases table/match ownership and sends IDLE immediately. Natural unseating follows the same cleanup. Historical roster/score indices remain fixed, and old match broadcasts cannot overwrite a new match. Living participants retain the jump lock; bot reuse stays under its original match cleanup.
...
- `WaterRespawn`, `AFKPortal`, `ChairShop`, `EggShops`, `LuckyBlocksTrigger` own their respective world interactions. WaterRespawn checks authored RespawnPoints in nearest-first order, then assigned/default spawns, and uses a supported dry landing with head clearance. Marker heights below the floor cannot embed a rig in the platform. Current contact and dry-floor checks reject stale touches/false water overlap after teleporting; a nearby immediate repeat excludes the failed destination for that recovery chain. If no safe authored landing exists, recovery reports it instead of repeating the same teleport. ScaleNPCWaterDestination and ScaleNPCWaterReturnCount expose bot return diagnostics. Bots retain only root touch detection; an existing-heartbeat feet/body check and bounded sweep also catch shallow water and fast falls. Before teleporting an ambient bot, recovery invalidates its movement token and clears seating reservations; it returns upright with cleared velocity and briefly pauses before choosing a new route. Failed pathfinding no longer falls back to a straight walk, and roaming ground selection rejects detected water. `CosmeticDisplay` and `ChairFx` own server-visible tags/chair effects and equipped-chair presentation.
...
The new-player seat guide, tutorial beam and arrow are removed. There is no tutorial UI, tutorial remote, forced bot match or saved tutorial state; ordinary Play Bot remains independent.

All hints use `MatchService.BuyHint -> GrantHint`: a pet charge or saved purchase credit grants the same owner-only `Hint.Actual` snapshot. PreviewController makes a local yellow highlighted clone at that exact size, sharing the reference/guess display conversion, the mystery's left edge and floor, and adaptive camera framing. Hints give no directional advice, never change the submitted guess, and leave mystery rulers unknown until reveal. GameConfig owns duration, use limits and appearance; expiry, locking, turn changes and teardown remove the clone.

During another player's guessing turn, one random available sabotage button at a time uses the hint button's shake/pop motion, without immediate repeats when several buttons are available. Wobble combines full camera spins and stronger sway with animated neon rings and orbiting sparks; an early release returns by the shortest angle. Ink fades while accelerating down the screen, including on an early turn lock. Both effects use server event ages, reuse their existing render loop and clean up on exit. `GameConfig.SabotageVisuals` owns the main presentation tuning.
